...
class ProgrammingParser(StandardParser):
    ...
    def make(self, *args, **kwargs):
        ...
        defStatement = self.keywords['def']('keyword') + (variable('function') + LPAREN + pp.delimitedList(PARAM)('parameters') + RPAREN
          ...
          | PARAM('parameter1') + PUNC('operator') + PARAM('parameter2')) + LBRACE + self.program('program') + RBRACE
        ...
        self.program <<= pp.ZeroOrMore(loadStatement)('loading') + pp.ZeroOrMore(statement).setParseAction(ProgramSequenceAction)
